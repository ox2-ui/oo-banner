    ...
    if (self.data.subscription) {
      self.autorun(function () {
        ...
        if (subscription.ready()) {
          ...
          dataSourceArray = Collection[self.data.collection].find({locationNames: self.data.location}).fetch()
...
Template.ooBanner.helpers({
  image: function () {
    console.log('%c Template.instance().activeImage.get()   ',  'background: #B3CC57; color: white; padding: 1px 15px 1px 5px;', Template.instance().activeImage.get());
